...
class ShapeAssemblyNet_A_vnn(nn.Module):
   ...
   def training_step(self, batch_data, device, batch_idx):
       self.iter_counts += 1
       partA_position, partA_rotation, total_loss, point_loss, rot_loss, trans_loss, recon_loss = self.forward_pass(batch_data, device, mode='train')
       return {"total_loss": total_loss,
               "point_loss": point_loss,
               "rot_loss": rot_loss,
               "trans_loss": trans_loss,
               "recon_loss": recon_loss,
               "predicted_partA_position": partA_position,
               "predicted_partA_rotation": partA_rotation
               }
   ...
   def forward_pass(self, batch_data, device, mode, vis_idx=-1):
       ...
       return pred_data['src_trans'], pred_data['src_rot'], total_loss, point_loss, rot_loss, trans_loss, recon_loss